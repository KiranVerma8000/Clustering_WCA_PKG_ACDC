...
class JavaClusterRunner:
    def __init__(self, jar_path, max_memory="14024m", algo="WCA", language="java",
                 deps_path="", measure="UEMNM", proj_name="", proj_version="",
                 proj_path="", stop_threshold=50, package_prefix="",stop = "preselected",serial_threshold=50,serial = "archsize"):
    ...
    def run(self):
        command = [
            ...
            f"stopthreshold={self.stop_threshold}", f"packageprefix={self.package_prefix}",f"stop={self.stop}",
            f"serial={self.serial}",f"serialthreshold={self.serial_threshold}"
        ]
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith(".rsf"):
            ...
            runner = JavaClusterRunner(
                ...
                stop=cluster_runner_template.stop,
                serial_threshold=cluster_runner_template.serial_threshold,
                serial=cluster_runner_template.serial
...
if __name__ == "__main__":
    ...
    output_clustering_directory = "./outputs/wca_uemnm_output"
    cluster_runner_template = JavaClusterRunner(
    jar_path="./assets/jars/arcade_core_clusterer.jar",
        max_memory="10240m",
        algo="WCA",
        language="java",
        measure="UEMNM",  # Update measure to UEMNM for WCA with UEMNM
